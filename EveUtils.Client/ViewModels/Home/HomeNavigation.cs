...
/// <param name="OpenCharacterSettings">The character settings dialog for an ESI character id.</param>
...
public sealed record HomeNavigation(
    ...
    Func<int, Task> OpenCharacterSettings,
    Func<int, string, Task> AllowScope,
    Func<Task> ImportFittings)
{
    public static readonly HomeNavigation None = new(
        _ => Task.CompletedTask, _ => { }, _ => Task.CompletedTask, (_, _) => Task.CompletedTask, () => Task.CompletedTask);
}
