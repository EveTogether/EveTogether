...
public interface IDialogService
{
    ...
    /// </summary>
    /// <param name="preselectedCharacterIds">
    /// Ticked before the pilot looks at the dialog (ET-216, widened to a list ET-221 follow-up) — a starting point
    /// they can add to or clear, never an automatic choice. An id that names nobody in <paramref name="options"/>
    /// or an option that is not <see cref="CharacterPickOption.Enabled"/> is silently skipped (no error) rather
    /// than dropping the rest. Leave null (or empty) for every caller that has no opinion here — the fleet
