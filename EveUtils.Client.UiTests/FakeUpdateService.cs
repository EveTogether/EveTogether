...
{
    /// <summary>
    /// What a check answers. Default: the feed answered and this build is current.
    /// </summary>
    public Func<Task<Result<AppRelease?>>> OnCheck { get; set; } =
        () => Task.FromResult(Result<AppRelease?>.Success(null));

    /// <summary>
    /// What a download answers. Default: it worked.
    /// </summary>
    public Func<Task<Result>> OnDownload { get; set; } = () => Task.FromResult(Result.Success());

    /// <summary>
    /// How often a check was asked for — so a test can assert one was skipped entirely.
    /// </summary>
    public int Checks { get; private set; }

    /// <summary>
    /// How often the downloaded package was applied.
    /// </summary>
