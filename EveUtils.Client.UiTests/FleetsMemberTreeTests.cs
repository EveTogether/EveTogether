...
public class FleetsMemberTreeTests
{
    ...
    public async Task FleetsWindow_WithSpeedFigures_Renders()
    {
        ...
        frame!.Save(System.IO.Path.Combine(System.IO.Path.GetTempPath(), "eveutils-fleets-speed-stats.png"), new Avalonia.Media.Imaging.PngBitmapEncoderOptions());
        window.Close();
    }
    ...
    public async Task FleetsWindow_MyFleetsTab_WithMemberLeaves_Renders()
    {
        ...
        var frame = window.CaptureRenderedFrame();
        Assert.NotNull(frame);
        frame!.Save(System.IO.Path.Combine(System.IO.Path.GetTempPath(), "eveutils-fleets-member-tree.png"), new Avalonia.Media.Imaging.PngBitmapEncoderOptions());
        window.Close();
    }
}
