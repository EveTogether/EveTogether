...
- **Fixed: Start run can now start exploration relic and data sites.** CCP never publishes these in the game data, so Start run had no way to find a site like "Detected Ruined Rogue Drone Science Outpost". Sites from your earlier runs now show up as suggestions with the kind they were recorded under, so one click starts them. A name you type yourself starts as well: pick what kind of site it is and go. A site the catalogue knows always wins over a typed name.
