...
- **Fixed: DPS no longer streams to the server while you are not in a fleet.** A character's remote DPS sample used to publish every 150 ms regardless of fleet state, reaching every connected client's `dps.html` page even while flying solo. It is now gated the same way fleet metrics already were: it publishes only while the character is an active participant of a server fleet, and stays quiet for a client-only fleet or no fleet at all.
