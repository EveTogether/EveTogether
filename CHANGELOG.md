...
- **Changed: Save in Settings no longer closes the window.** It now applies and waits, then shows "Saved." (or "Could not save: …" if it failed) right there in the footer, so a failed save is visible instead of silently vanishing behind a closed window. Close the window with Cancel or the close button, same as before. Re-import still closes, since the SDE popup that follows takes over the screen anyway.
