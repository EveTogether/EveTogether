...
- **Fixed: the button beside the tab strip now pops out just the current tab, instead of duplicating the rail's dock/float switch.** It used to run the same all-modules dock/float toggle as the FLOAT button at the bottom of the rail, so clicking it beside a tab switched every open module to a window instead of just the one you were looking at. It now pops only that tab out, leaving the dock mode and every other open module untouched, with its own icon and tooltip. A DOCK button in the pop-out window's own titlebar brings it back into the tab strip.
- **Fixed: a day you lost ISK on no longer looks empty in the runs calendar.** A day that netted a loss — a failed abyssal outweighing the day's homefronts — was drawn like a day with nothing in it. It now shows in red (violet on Minmatar, where the accent itself is red), with a thin ring inside the cell so it also reads without the colour, and the ISK legend gets a loss side. Days with activity but no known net show a small dot, and the SUMMARY hours and days and the LAST 30 DAYS bars on Home follow the same rule, with losses hanging below a zero line.
- **Fixed: picking a day in the runs calendar no longer shows its header twice.** The pinned header above the list now appears only once the day's own header has scrolled out of view.
- **Fixed: deletes from the server's control panel now reach connected pilots right away.** Deleting a composition, a shared fit or a fleet in the control panel used to leave it on everyone's screen until they reloaded. A fleet the server retires after it has gone quiet also stayed in the list. These changes now reach everyone who has the fleet, the composition or the fit library open, the same way a pilot's own delete does. The server needs updating for this; with an older server you still reload by hand.
- **Fixed: the Local fits list keeps up with changes made in other windows.** Deleting, renaming or downloading a fit now updates the Local list, whichever window you did it from.
- **Fixed: a fit you import now shows up in an open FITS list, and a killmail you link or unlink updates an open KILLMAILS window.** Importing a copied fit from the clipboard offer used to leave the Local list without it until you reopened the tab, and linking a loss to a run from the run window left KILLMAILS showing it under "Not linked to a run". Both screens now follow the change as it happens.
- **Fixed: wings, squads and accepted join requests now show up live for the whole fleet.** Adding, renaming or deleting a wing or squad, or accepting someone's request to join, used to leave the other members looking at the old roster until they reopened it. Every change to a fleet — its roster, its structure, its invites — now reaches everyone who has that fleet open. Needs the server updated; against an older server you still refresh by hand.
- **Fixed: Home now follows a local fleet the moment you start or stop it.** Starting a local fleet used to leave Home showing it as forming, with a START button, until something unrelated refreshed the page; stopping, concluding, the automatic stop, creating and disbanding had the same blind spot. Home and the Fleets window now pick up every change to a local fleet as it happens.
- **Added: homefronts are recognised from the enemies you fight.** While you are in a fleet, the app watches the game log for the NPCs and structures that belong to one particular homefront — the Offertory Sigils of a Raid, the dreadnought you cap in a Dread Assault, the arrays you neut in Stabilize Rift — and asks "Start Homefront run: <site>?". One click on Start opens the run on that site, with your characters in the fleet already along for it. You are asked once per site; Ignore stays ignored, and nothing is asked while you already have a run going. Metaliminal Meteoroid, Abyssal Artifact Recovery, Salvage Research, Traffic Stop and the start of a Suspicious Signal have nothing this can recognise yet, so copying the site from the probe scanner is still the way there. Turn it off under Settings with "Offer homefront runs from enemies seen".
- **Fixed: editing a fleet now shows up live for everyone who can see it.** Making a public fleet invite-only, or renaming or rescheduling one, used to leave other pilots looking at the old row in the fleet list until they pressed Refresh — and the row they could no longer join stayed put. The change now reaches every connected pilot when the fleet is public (before or after the edit), and only its members otherwise. Needs the server updated; against an older server you still refresh by hand.
...
- **Changed: a new home screen that shows the whole app at a glance.** What you made today, this week and this
  ...
  ALLOW… on hover, never a made-up zero.
...
- **Added: MINING now groups by character, with a share bar, ISK/h and residue.** Instead of one flat line per
  ...
  A fleet mate on a client new enough to send the split gets the same per-ore breakdown as your own characters; an
  older client still shows their total, and an external member with nothing shared at all shows as "not shared" and
  is left out of the total, the same as FLEET already does. A local character who runs a Mining Foreman Burst gets a
  ▲▲ next to their name; other local characters get a ▲ while that burst is still recent — inferred from the
  booster's own log, since a receiver's log never says anything about a burst landing on them, so it is never shown
  as certain. Solo no longer shows a line about what "the fleet" mined — there isn't one.
