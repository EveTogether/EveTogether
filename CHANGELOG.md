...
- **Added: EVE Workbench as a second appraisal price source, chosen from the Appraisal tool.** Paste a personal access token (from EVE Workbench under My account → Personal access tokens) to unlock it; MAKE DEFAULT sets your pick for every screen that prices loot, mining and consumables, not just the tool itself. The token is encrypted on this machine and never shown again once saved. Without a token, or if EVE Workbench cannot be reached, run and mining valuations fall back to the ESI average and say so.
