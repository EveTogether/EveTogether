...
- **Fixed: the clipboard-watch setting no longer claims to be Windows only.** It also works on Linux under Wayland with `wl-clipboard` installed; the disclosure that appears when the feature is unavailable now says so, instead of the outdated "Windows only" — and the always-visible line below it now says "your system" instead of naming Windows specifically.
