...
/// <summary>
/// ET-78: a refused fleet RPC answers with <see cref="StatusCode.Unauthenticated"/> — the same signal
/// <c>EventBusStreamService.Attach</c> already raises — instead of an <c>Ok = false</c> reply payload. That status
/// is what the client's <c>InvokeAsync</c> watches for to refresh the session and retry the call in flight; a reply
/// payload sailed straight past it, leaving the call lost until the next 30s heartbeat (the gap ET-77 left open).
/// </summary>
public class FleetsUnauthenticatedStatusTests
{
    private readonly SqliteServerDbContextFactory _factory = new();

    [Fact]
    public async Task AMissingBearer_IsRefusedWithUnauthenticated_NotAnOkFalseReply()
    {
        var service = Service();

        var thrown = await Assert.ThrowsAsync<RpcException>(
            () => service.CreateFleet(new CreateFleetRequest { Name = "Roam" }, Context(bearer: null)));
    ...
    [Fact]
    public async Task AnUnknownToken_IsRefusedTheSameWayOnAListRead_NotAsAnEmptyList()
    {
        var service = Service();

        var thrown = await Assert.ThrowsAsync<RpcException>(
            () => service.ListMyFleets(new ListMyFleetsRequest(), Context(bearer: "no-such-token")));
    ...
    private FleetsGrpcService Service() =>
        new(new ServerSessionService(new ServerAuthRepository(_factory), NullLogger<ServerSessionService>.Instance),
            null!, null!, null!, null!, null!, new FleetActivityTracker(new ServiceCollection().BuildServiceProvider()), null!);
