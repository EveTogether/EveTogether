...
public class BackupRoundTripTests : IDisposable
{
    ...
    private async Task SeedAsync(string characterName = "Jithran")
    {
        ...
        db.Add(new BackupDownload
        {
            ...
            AppVersion = "0.2.0",
            FileName = "eve-together-backup-20260830-090000Z.etbackup",
        });
        await db.SaveChangesAsync(TestContext.Current.CancellationToken);
    }
    ...
    private static byte[] EditATableEntry(byte[] archive) =>
        RewriteZip(archive, zip =>
        {
            var entry = zip.Entries.First(e => e.FullName.StartsWith(BackupFormat.DatabaseEntryPrefix, StringComparison.Ordinal)
                                               && e.Length > 0);
            var name = entry.FullName;
            entry.Delete();
            using var replacement = zip.CreateEntry(name).Open();
            replacement.Write("not what the manifest says"u8);
        });

    private static byte[] WithoutTheTokenProtectorKey(byte[] archive) =>
        RewriteZip(archive, zip =>
        {
            zip.GetEntry(BackupFormat.DataEntryPrefix + BackupFormat.TokenProtectorKeyFile)?.Delete();

            // The manifest has to stop listing it too, or the checksum pass reports a missing entry instead.
            var manifestEntry = zip.GetEntry(BackupFormat.ManifestEntry)
                ?? throw new InvalidOperationException("The archive has no manifest.");
            BackupManifest manifest;
            using (var read = manifestEntry.Open())
                manifest = System.Text.Json.JsonSerializer.Deserialize<BackupManifest>(read, BackupJson.Options)!;

            manifest.Files.RemoveAll(f => f.Name == BackupFormat.TokenProtectorKeyFile);
            manifestEntry.Delete();
            using var write = zip.CreateEntry(BackupFormat.ManifestEntry).Open();
            write.Write(System.Text.Json.JsonSerializer.SerializeToUtf8Bytes(manifest, BackupJson.Options));
        });

    private static byte[] RewriteZip(byte[] archive, Action<ZipArchive> edit)
    {
        var plaintext = new MemoryStream();
        using (var reader = BackupEnvelope.OpenReader(new MemoryStream(archive), Password))
            reader.CopyTo(plaintext);

        using (var zip = new ZipArchive(plaintext, ZipArchiveMode.Update, leaveOpen: true))
            edit(zip);

        var rebuilt = new MemoryStream();
        using (var envelope = BackupEnvelope.CreateWriter(rebuilt, Password))
            envelope.Write(plaintext.ToArray());
